...
export const AudioControls: React.FC<AudioControlsProps> = ({ isPlaying, onToggle, available, rate, onRateChange }) => {
    ...
    return (
        <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-full px-2 py-1 shadow-inner">
                 <button 
                    onClick={() => onRateChange(Math.max(0.5, rate - 0.1))}
                    className="text-xs font-bold text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 w-5 h-5 flex items-center justify-center"
                >-</button>
                <span className="text-xs font-mono text-gray-700 dark:text-gray-300 w-8 text-center">{rate.toFixed(1)}x</span>
                <button 
                    onClick={() => onRateChange(Math.min(2.0, rate + 0.1))}
                    className="text-xs font-bold text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 w-5 h-5 flex items-center justify-center"
                >+</button>
            </div>
        </div>
